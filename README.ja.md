...
| Visualizer size | ドラッグインジケーターの大きさを調整します。 |
| Liquid Glass | ガラス風のドラッグ表示を調整します。 |
| Launch at Login | サインイン時にMac Drag Scrollを自動起動します。 |
| Excluded Apps | 選択したアプリではドラッグスクロールを無効にします。 |

設定はmacOSユーザーごとに`~/Library/Preferences/com.martincalander.macdragscroll.plist`へ保存されます。通常のアプリ削除や再インストールではこのファイルは削除されないため、設定はアンインストールやアップグレード後も残ります。

...
- Strict warnings, Address Sanitizer tests, dependency review, governance, and two-person review now have repository evidence for the maintainer to assess in the Silver/Gold forms.
...
The required Quality Gate performs dependency review, strict warning-free tests, code coverage reporting, Address Sanitizer testing, Xcode static analysis, and universal release compilation. The resulting executable must include both `arm64` and `x86_64` slices and declare macOS 14.0 as its minimum in both its bundle metadata and Mach-O load commands.

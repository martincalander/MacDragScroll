...
struct TriggerConfig: Codable, Equatable {
    ...
    static func captured(button: Int, modifiers: NSEvent.ModifierFlags) -> TriggerConfig {
        ...
        if button <= 1 && !config.hasModifiers {
            config.requiresCommand = true
        }
...
class SettingsManager: ObservableObject {
    ...
    @Published var launchAtLogin: Bool {
        didSet {
            guard !isSynchronizingLaunchAtLogin, launchAtLogin != oldValue else { return }
    ...
    private func setLaunchAtLogin(_ enabled: Bool) {
        do {
            ...
            let actualStatus = getLaunchAtLoginStatus()
            guard launchAtLogin != actualStatus else { return }
